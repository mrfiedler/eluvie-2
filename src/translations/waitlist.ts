
import { Translation } from './types';

const waitlistTranslations = {
  'join-waitlist': {
    en: 'Join Our Waitlist',
    'pt-BR': 'Entre na Nossa Lista de Espera'
  },
  'be-first': {
    en: 'Be the first to know when Eluvie launches. Get early access and exclusive offers!',
    'pt-BR': 'Seja o primeiro a saber quando o Eluvie for lançado. Obtenha acesso antecipado e ofertas exclusivas!'
  },
  'thank-you': {
    en: 'Thank you for joining!',
    'pt-BR': 'Obrigado por se juntar a nós!'
  },
  'notify-launch': {
    en: "We'll notify you as soon as Eluvie launches.",
    'pt-BR': 'Notificaremos você assim que o Eluvie for lançado.'
  },
  'submitting': {
    en: 'Submitting...',
    'pt-BR': 'Enviando...'
  },
  'join-the-waitlist': {
    en: 'Join the Waitlist',
    'pt-BR': 'Entrar na Lista de Espera'
  },
  'payment-received': {
    en: 'Payment Received',
    'pt-BR': 'Pagamento Recebido'
  },
  'payment-info': {
    en: 'Your payment of $1,250 has been processed successfully',
    'pt-BR': 'Seu pagamento de R$6.250 foi processado com sucesso'
  }
};

export default waitlistTranslations;
